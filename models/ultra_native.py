...
class UltraNativeEnhancer(BaseEnhancer):
    def enhance_frame(self, frame):
        ...
        edges = cv2.GaussianBlur(edges, (0,0), 1.8)
        frame = cv2.addWeighted(frame, 1.0, cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR), 0.45, 0)
        return super().enhance_frame(frame)

    def get_ffmpeg_vf(self, tw, th):
        vf = super().get_ffmpeg_vf(tw, th)
        return f"cas=0.9,hqdn3d=5:5:4:4,unsharp=7:7:2.0,{vf}"